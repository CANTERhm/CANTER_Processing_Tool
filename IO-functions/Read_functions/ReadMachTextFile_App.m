function [mach_data,segment_headers] = ReadMachTextFile_App(app,varargin)
    %%  ReadMachTextFile:
    %   This function reads text files created from the Mach-1 indenter
    %   system from Biomomentum.
    %   
    %   [mach_data,segment_infos] = ReadMachTextFile_App();
...
%     wb = waitbar(0,'Please wait ... read header information');
    wb = uiprogressdlg(app.figure1,...
                       "Indeterminate","on",...
                       "Message","Please wait ... reading header information",...
...
%     waitbar(i/(2*segment_count),wb,'Please wait ... read data channels');
    wb.Message = "Please wait ... reading data channels";
